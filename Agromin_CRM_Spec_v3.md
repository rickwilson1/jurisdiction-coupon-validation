...
### CRM Evaluation, Sales Inbox Specification & ERP Horizon Assessment

| | |
|---|---|
| **Prepared by** | Rick Wilson, Ph.D. MBA |
| **Date** | April 2026 |
...
- **Phase 1** (email triage + order dispatch) can be built immediately at $0 incremental cost using existing Microsoft 365 licenses and existing Google Cloud infrastructure (Cloud Run, Firestore, Vertex AI).
- **Phase 2** (CRM) requires a formal evaluation. Sage 100 native integration is the make-or-break criterion for any CRM selected while Agromin remains on Sage 100.
- **The ERP question must be answered first.** If Agromin migrates to a cloud ERP within 2–3 years, the CRM selection changes materially. **Microsoft Business Central** is the recommended cloud ERP path given Agromin's existing Microsoft investment — and it resolves the CRM question simultaneously by making D365 Sales the natural choice at no additional middleware cost.
- **If Sage 100 remains the platform**, Sage CRM is the only native integration candidate at reasonable cost and is the leading CRM option in that scenario.
w
**Decisions Requested**
...
3. **Answer the ERP horizon question** — is Sage 100 a 5-year platform or is a cloud ERP migration on the roadmap? This determines the CRM evaluation path.
4. If staying on Sage 100: authorize Sage CRM and D365 Sales demos within 30 days.
5. If pursuing cloud ERP: scope a Business Central evaluation alongside D365 Sales as a combined platform decision.
6. Designate a CRM/ERP decision owner and target decision date.
...
| Vertex AI | Google | Gemini 1.5 Flash. Used for email classification in Workstream A. Same GCP project as dispatch service. | Active |
...
- Classifies each email by category (order confirmation, RFQ, new business inquiry, customer service, spam, etc.) using Vertex AI Gemini 1.5 Flash (GCP)
...
| AI classification | Vertex AI Gemini 1.5 Flash via HTTP connector (GCP) | ~$0.01/email |
...
- Sends the correct instructions email to the customer automatically — sent from `dispatch@agromin.com`
- Alerts the delivery coordinator on every delivery order — sent from `dispatch@agromin.com`
...
| Sending mailbox | `dispatch@agromin.com` — M365 shared mailbox (Send As) | $0 — no license required |
...
| 2 | **Total cost (license + middleware)** | License price alone is misleading. Evaluate fully-loaded cost: license + middleware + implementation amortized over 12 months. Vendor must provide written quote. | HIGH |
...
| 8 | **Dispatch system API** | The order dispatch service (Workstream B) needs to write operational records to the CRM programmatically. REST API with custom field support required. If absent, Firestore serves as a separate fallback at $0 cost — not a blocker for CRM selection but is a factor. | MEDIUM |
...
## 5. ERP Horizon & Cloud ERP Evaluation

### 5.1 Why the ERP Question Must Be Answered First

The CRM recommendation in this document is optimized for Agromin's current ERP: **Sage 100**. Before committing to a Sage-native CRM, leadership must answer one question:

> **Is Sage 100 a 5-year platform for Agromin, or are we likely to outgrow it?**

This matters for two reasons:

1. Sage CRM's primary advantage — native Sage 100 sync — disappears if Agromin migrates to a cloud ERP within 2–3 years. The switching cost would be paid twice.
2. A cloud ERP migration would resolve the CRM middleware problem permanently. Most modern cloud ERPs have native connectors to stronger CRM platforms that Sage 100 cannot support cost-effectively today.

### 5.2 Signs Sage 100 May Have a Limited Horizon

- Growing complexity in multi-entity operations (Agromin + California Compost) that Sage 100 handles with workarounds
...
- Limited API surface makes automation, reporting, and third-party integration progressively harder over time

### 5.3 Recommended Cloud ERP: Microsoft Business Central

Of the available cloud ERP options, **Microsoft Business Central is the best fit for Agromin** given its existing technology stack.

| ERP | Fit for Agromin | Cost | Key Factor |
|---|---|---|---|
| **Microsoft Business Central** | **Best fit** | ~$70–100/user/mo | Native M365 + D365 integration; solves CRM simultaneously |
| Sage Intacct | Good — same vendor | ~$400–700/mo base | Strong financials; weaker on distribution operations |
| NetSuite | Strong operationally | ~$1,000–2,000+/mo | Most powerful; most expensive; new vendor relationship |
| Acumatica | Good for distribution | Consumption-based | Strong ops; less common in this sector |

**Why Business Central for Agromin:**

- **M365 is already deployed** — Business Central is a natural extension of the same Microsoft tenant. Teams, Outlook, SharePoint, Power Automate, and Power BI all connect natively
- **Resolves the CRM question** — D365 Sales + Business Central = one vendor, native bidirectional sync, no middleware ever. The $500/month Commercient SYNC cost disappears
- **CIMcloud connector exists** — CIMcloud has a Business Central integration, preserving the customer portal without a rebuild
- **Power Automate is native** — the order dispatch system and email triage workflows already built on Power Automate continue without modification
- **Azure OpenAI is already provisioned** — Copilot for Business Central is Azure OpenAI-powered, meaning the idle AI resource becomes productive
- **Blytheco partnership** — Blytheco (Agromin's current Sage partner) is also a Microsoft partner. The existing relationship may carry forward
- **Two-entity support** — Business Central handles Agromin and California Compost as separate legal entities within one system

**What Business Central migration would look like:**
- Implementation timeline: typically 3–6 months for a company at Agromin's scale
- Blytheco or a Microsoft VAR handles data migration from Sage 100
- CIMcloud continues operating during transition
- D365 Sales deployed simultaneously — one project, one vendor, one go-live

### 5.3a Business Central — Estimated Migration & Ongoing Costs

These are typical ranges for a company at Agromin's scale (~20–50 employees, two legal entities, distribution/agriculture operations). Actual quotes from a Microsoft VAR are required before a decision.

**One-Time Implementation Costs**

| Item | Low Estimate | High Estimate | Notes |
|---|---|---|---|
| Implementation & configuration | $25,000 | $60,000 | VAR-led; complexity driven by chart of accounts, inventory setup, and custom workflows |
| Data migration (Sage 100 → BC) | $5,000 | $15,000 | Customer master, vendor, open AR/AP, item catalog; historical transactions usually excluded |
| CIMcloud connector setup | $2,000 | $6,000 | CIMcloud has a native BC integration; configuration cost only |
| D365 Sales setup | $3,000 | $8,000 | Usually included with BC if same VAR; separate if phased |
| Training (staff) | $2,000 | $5,000 | End-user and admin training; typically 2–3 days |
| Contingency (10–15%) | $4,000 | $14,000 | Standard buffer for scope creep |
| **Total one-time** | **~$41,000** | **~$108,000** | Midpoint estimate: ~$65,000–75,000 |

**Ongoing Monthly Costs (Post Go-Live)**

| Item | Cost | Notes |
|---|---|---|
| Business Central Essentials | ~$70/user/mo | Standard financial + supply chain. 10 users = $700/mo |
| Business Central Premium | ~$100/user/mo | Adds manufacturing + service management. Use only for users who need it |
| D365 Sales Professional | ~$65/user/mo | 8 sales reps = $520/mo |
| Power Automate Premium | $0 | Already licensed; carries forward |
| Azure OpenAI (Copilot) | Included | Provisioned; Copilot for BC consumes existing resource |
| VAR support retainer | $500–1,500/mo | Ongoing admin support, patches, minor customizations |
| CIMcloud (unchanged) | Current cost | No change to CIMcloud subscription |
| **Total monthly (10 BC users + 8 sales)** | **~$1,720–2,720/mo** | Includes VAR retainer; excludes CIMcloud |

**Comparison: Current Sage 100 Annual Cost vs. Business Central**

| Cost Element | Sage 100 (estimated) | Business Central |
|---|---|---|
| ERP license/maintenance | ~$8,000–15,000/yr | ~$8,400–12,000/yr (10 users Essentials) |
| CRM (if adding) | ~$4,300/yr (Sage CRM) | ~$6,240/yr (D365 Sales, 8 users) |
| CRM middleware | $0 (native) | $0 (native) |
| VAR support | ~$6,000–18,000/yr | ~$6,000–18,000/yr |
| Integration tax (custom connectors) | Growing | Eliminated |
| **Annual platform cost** | **~$18,000–37,000/yr** | **~$21,000–37,000/yr** |

The ongoing cost delta between platforms is small — roughly $3,000–5,000/year more for Business Central at comparable user counts. The migration is a one-time expense of ~$65,000–75,000. The break-even is approximately 4–6 years of eliminated integration tax and middleware before considering the operational value of a more capable platform.

**Key variables that move the implementation number:**
- Complexity of inventory and costing setup (standard vs. lot tracking, multiple warehouses)
- Number of years of historical data to migrate
- Degree of custom reporting or workflow required
- Whether CIMcloud and D365 Sales are deployed simultaneously or phased

### 5.4 The Two Paths and What They Mean for CRM

| Scenario | ERP | CRM | Middleware | Monthly Platform Cost |
|---|---|---|---|---|
| **Stay on Sage 100** | Sage 100 | Sage CRM | $0 | ~$360/mo (CRM only) |
| **Migrate to Business Central** | Business Central | D365 Sales | $0 | ~$1,200–1,600/mo (ERP + CRM) |
| Stay on Sage 100 + D365 | Sage 100 | D365 Sales | ~$500/mo | ~$1,580/mo (CRM + middleware only) |

The Business Central path costs more monthly but delivers an integrated ERP + CRM + Microsoft ecosystem with no middleware. The Sage 100 + D365 path pays middleware cost without the ERP benefit. The Sage 100 + Sage CRM path is lowest cost but accepts a limited platform.

### 5.5 Recommendation

**If Agromin's leadership believes Business Central is the right direction within 3 years** — scope a combined ERP + CRM evaluation now. Do not invest in Sage CRM. The right demo sequence is Business Central + D365 Sales as a unified platform decision.

**If Sage 100 is firmly the 5-year platform** — proceed with Sage CRM evaluation as described in Section 8. The native sync is a genuine operational advantage worth optimizing for at that time horizon.

**If the answer is uncertain** — answer this question before committing to any CRM. A 30-day pause is cheaper than buying a platform optimized for a system you're about to leave.
...
## 7. Dispatch System and CRM — Integration Approach
...
**If selected CRM has a workable REST API:**
The dispatch service writes directly to the CRM. Order routing decisions, scheduled delivery dates, customer details, and fulfillment status sit alongside the full customer record in one system. The delivery coordinator manages delivery workflow inside the CRM using its built-in views and fields.
...
### 8.1 Phase 1 — Proceed Immediately (Both Workstreams)

**Workstream A — Sales email triage:** Authorize build of the `sales@agromin.com` triage and routing system. No new budget required. Uses existing M365 licenses, Power Apps Premium seat (already assigned), and Vertex AI Gemini 1.5 Flash (GCP). 4-week build timeline.
...
### 8.2 Phase 2 — CRM Evaluation and Decision Path
...
If Sage CRM's UX drives poor adoption or the API is insufficient, **do not default to an expensive platform.** The correct fallback is a lightweight custom-built operational tool on the existing GCP stack.

What this covers:
- Delivery coordinator queue — pending and scheduled deliveries in one view
- Schedule form — coordinator logs confirmed date/time, triggers customer email automatically
- Operations schedule view — live read-only dashboard
...
For the sales CRM function (pipeline, lead management, reps), D365 remains a viable option if Agromin is prepared to accept the $1,580/month fully-loaded cost and the Microsoft commercial stack commitment.

---

**Stage 3 — If the integration tax becomes unsustainable: Evaluate ERP migration**

If Agromin reaches a point where every new tool requires custom middleware because Sage 100's API is too limited — that is the signal to evaluate a cloud ERP migration, not to keep adding connectors.
...
This is not a recommendation to migrate now. It is a recognition that the correct response to a failing integration stack is an ERP decision, not another middleware purchase.
...
### 8.3 CRM Selection Criteria for Dispatch Integration
...
A score of 3 or below on Criterion 8 does not disqualify a CRM — Firestore and a custom-built operational layer serve as the fallback at zero cost. But a strong API score eliminates the need for custom development and is preferred.
...
| 6 | Request Sage CRM demo — Sage 100 sync walkthrough, REST API demo, fully-loaded cost proposal | CRM decision owner | Apr 18, 2026 | PENDING |
| 7 | Request D365 Sales demo — for cost and feature comparison against Sage CRM | CRM decision owner | Apr 18, 2026 | PENDING |
...
*Agromin — Confidential and Proprietary. April 2026. Version 3.0*
